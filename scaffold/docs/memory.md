...
`ID:`, `Type:` or `Parent:`. That is a warning under `scripts/check-memory.sh
...
stdout. Nothing generated by `scripts/git-memory-graph.sh` or
`scripts/git-memory-packet.sh` is committed, and a `build/` directory used to
...
`scripts/check-memory.sh` already checks.
...
`./scripts/check-memory.sh --fix`; hand-editing it is what rule 3 below forbids.
...
Four rules prevent two layers from drifting apart. `scripts/check-memory.sh` enforces what is mechanically checkable.
